...
void createQuadL(int i, char *b, char *c, char *d)
{
	...
	{
	case 1:
		createQuad("BNZ", convert(qc + 3), b, "");
		createQuad("=", "1", "", d);
		createQuad("BR", convert(qc + 2), "", "");
		createQuad("=", "", "0", d);
		break;
	case 2:
		createQuad("BNZ", convert(qc + 4), b, "");
		createQuad("BNZ", convert(qc + 3), c, "");
		createQuad("=", "0", "", d);
		createQuad("BR", convert(qc + 2), "", "");
		createQuad("=", "1", "", d);
		break;
	case 3:
		createQuad("BZ", convert(qc + 4), b, "");
		createQuad("BZ", convert(qc + 3), c, "");
		createQuad("=", "1", "", d);
		createQuad("BR", convert(qc + 2), "", "");
		createQuad("=", "0", "", d);
		break;
	...
	for (i = 0; i < qc - 1; i++)
	{
		if (strcmp(liste[i].opr, "=") == 0)
		{
			...
			for (j = i + 1; j < qc; j++)
			{
				...
				{
					nbQc++;
					trouve = 1; // trouve est utilise pour la condition if
